...
### 规则 4：WebUI 为一等公民（跨平台主界面）

- **背景**：工具面向三平台（Windows / Linux / macOS），WPF 仅限 Windows，而 WebUI 在所有平台上运行一致
- **规则**：**WebUI 是永久的一等公民交互形态**，而非仅初版验证介质。所有新交互功能优先在 WebUI 中实现，确保跨平台可用
- **WPF 定位**：WPF/WinUI 是 **Windows 增强壳**（提供原生托盘、资源管理器定位等 WebUI 无法实现的能力），不替代 WebUI
- 不因 WPF 已就绪而降低 WebUI 的完整度；WebUI 和 WPF 在功能上对齐，WebUI 优先保障跨平台完整
