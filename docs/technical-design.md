...
6a. **WebUI 为一等公民，跨平台主界面**：WebUI 是工具的**永久一等公民交互形态**，而非仅初版验证介质。理由：工具面向三平台（Windows/Linux/macOS），WPF 仅限 Windows，而 WebUI 在所有平台上运行一致。因此 WebUI 是**跨平台主界面**，WPF/WinUI 是**Windows 增强壳**（提供原生托盘、资源管理器定位等无法通过 WebUI 实现的能力）。此规则长期有效：所有新交互功能优先在 WebUI 中实现（确保跨平台可用），WPF 在 Windows 上做增强复刻，但不因 WPF 就绪而降低 WebUI 的完整度。
...
| 层 | 选型 | 通信模式 | 决策理由 |
|---|---|---|---|
| **UI 层** | WebUI（Vite + Vue，跨平台主界面）+ WPF（Windows 增强壳） | HTTP 3000 / 进程内 | **WebUI 为一等公民**：跨平台一致体验；WPF 提供 Windows 原生增强（托盘、资源管理器定位），不替代 WebUI |
...
> **定位（设计原则 6a）：** WebUI 是工具的永久一等公民交互形态，面向三平台（Windows / Linux / macOS）提供一致的交互体验。所有新交互功能优先在 WebUI 中实现，确保跨平台可用。WPF 作为 Windows 增强壳，不替代 WebUI 的跨平台角色。
...
### 5.4 桌面 GUI（WPF，Windows 增强壳）

**定位：** WPF 是 **Windows 增强壳**，提供 WebUI 无法实现的原生系统集成（系统托盘、资源管理器定位、后台扫描通知）。WebUI 是跨平台一等公民；WPF 在 Windows 上增强体验，不替代 WebUI 的跨平台角色。v1.0 以 Windows 为目标，跨平台桌面壳（Avalonia/Tauri）为后续评估项。
