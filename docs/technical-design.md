...
6a. **展示层 UI 一律先 WebUI 验证交互（WebUI-First）**：任何展示层交互需求，一律先通过 WebUI 验证交互逻辑的合理性与用户体验，经真实使用确认满意后，再考虑 WPF/WinUI 桌面壳落地。WebUI 是"交互实验室"，WPF/WinUI 是"成熟交互的复刻落地"——不反向。（此规则指导切片 7/8 顺序，亦适用于后续任何新交互需求的开发路径。）
...
| 层 | 选型 | 通信模式 | 决策理由 |
|---|---|---|---|
| **UI 层** | WebUI（Vite + Svelte/Solid）+ WPF（桌面壳 Windows 优先） | HTTP 3000 / 进程内 | **WebUI-First**：交互一律先经 WebUI 验证，满意后 WPF 复刻（设计原则 6a） |
