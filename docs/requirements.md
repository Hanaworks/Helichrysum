...
>
> **报告交互化（在报告内直接标记/决策）为 v1 后评估项**：先交付只读版经真实使用评估，再决定是否演进。
...
| F-Report-6 | 必须 | 报告为**单一形态 + 数据分级渲染**：摘要（目录结构聚合 + 关系汇总 + 标签统计）内嵌，详情按需惰性加载（虚拟滚动 + 聚合下钻）。数据量 ≤ 可配阈值时导出为**单文件 HTML**（自包含、可分享、离线打开）；超限时提供**报告服务地址**（`report serve`，同一渲染机制，数据源切本地服务按需取）——二者二选一，不混合 |
