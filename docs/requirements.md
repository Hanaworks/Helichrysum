...
| F-Resolve-4 | 必须 | `Compatibility` 判定的证据：新版本 mtime ≥ 旧版本、新版本 size ≥ 旧版本、文件列表包含关系 |
...
| F-Resolve-11 | 必须 | **自上而下的按需下钻决策**：决策以**目录级 → 文件级 → 内容级**为主路径（从最省事的粗粒度开始）。仅当上级已在分析中发现"需要下级裁决"的不确定点（如新旧交织、部分重叠、结构相似但文件列表有出入）时，才下钻到更细粒度解决；下钻完成后如需修正上级结论，则回溯重算上级——**一般情况下从上到下，处理且有必要时才从下到上** |
| F-Resolve-12 | 必须 | **交织处理规则**：即使两个目录整体判定为 A 新 B 旧，B 中仍可能存在"比 A 更新"的文件（新旧交织）。上级判定存疑时必须下钻到文件级独立解决每个文件的新旧冲突（合并/以新为准/KEEP），再回溯到目录级基于文件级解决后的状态重新计算整体关系，不得用目录级判断覆盖文件级已解决的问题 |
...
| F-Resolve-14 | 必须 | **依赖链可视化**：报告必须能体现决策的层级依赖关系——每个对象/目录的处理结果标注"由哪一层、基于哪些下层物化结果得出"，用户可沿依赖链追溯（如：目录级判定 ← 文件级已解决 ×N） |

> **设计主旨**：只人工确认必要的部分；但不需要确认的部分也必须列出来——自动化不隐身，全部可见、可回滚、可否决。
> **执行模型**：扫描阶段全部并行（探测无依赖）；决策阶段**从上到下**（目录 → 文件 → 内容）为主路径，仅当上级存疑、需要下级裁决时才**按需下钻**，必要时**回溯**修正上级——从下到上只是例外路径，不是默认流水线。
