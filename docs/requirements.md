...
| F-Resolve-11 | 必须 | **分层决策流水线**：决策按粒度从细到粗分层执行——文件级 → 目录级 → 结构级。**扫描可并行，但决策处理必须逐层串行**：上一级的结果（物化后）是下一级的输入，下一级必须等到上一级全部解决后才能开始 |
| F-Resolve-12 | 必须 | **交织处理规则**：即使两个目录整体判定为 A 新 B 旧，B 中仍可能存在"比 A 更新"的文件（新旧交织）。文件级先独立解决每个文件的新旧冲突（合并/以新为准/KEEP），目录级再基于文件级解决后的状态重新计算整体关系，不得用目录级判断覆盖文件级已解决的问题 |
...
> **执行模型**：扫描阶段全部并行（探测无依赖）；决策阶段按依赖链串行分层（文件 → 目录 → 结构），每层物化后才进入下一层。
