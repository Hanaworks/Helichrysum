...
| F-Layered-7 | 必须 | **索引时持久化文件信息指纹**：扫描建索引时必须为每个文件存储信息指纹——快速摘要（CRC32/xxhash，必存）+ 强摘要（MD5 等，按 F-Layered-6 升级路径）+ 时间戳（mtime/ctime）。指纹随 manifest 持久化，作为后续**执行前验证、增量扫描、完整性校验**的统一基线；未存指纹的对象在执行阶段视为"基线缺失"，默认中止动作 |
