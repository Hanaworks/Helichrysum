...
| F-Layered-7 | 必须 | **索引时持久化文件信息指纹**：工具**具备**为文件存储信息指纹的能力——快速摘要（CRC32/xxhash）+ 强摘要（MD5 等，按 F-Layered-6 升级路径）+ 时间戳（mtime/ctime），随 manifest 持久化，作为**执行前验证、增量扫描、完整性校验**的统一基线。**指纹索引强度/是否全量存储是可配项**（按配置哲学：设置界面阶段定义等级），未存指纹的对象在执行阶段视为"基线缺失"，默认中止动作 |
...
| F-Report-12 | 必须 | **Manifest 快照元数据（过期感知）**：manifest 为**时间快照**，库级元数据必须记录：<br>• `created_at`（扫描开始）与 `scanned_at`（扫描完成，长扫描可能跨天）<br>• `scope_snapshot`（当时的 Scope 完整定义：根路径 + 排除规则）<br>• `tool_version`（生成它的工具版本）<br>**报告与 UI 必须展示快照年龄**（如"快照创建于 2 周前，数据可能已变化"），处理前提示用户必要时重扫；快照生成新快照时可与其对比（增量扫描基础），过期快照执行处理时依赖 F-Exec-11 指纹校验兜底 |
